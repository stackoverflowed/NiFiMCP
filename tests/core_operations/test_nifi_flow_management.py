...
async def test_document_flow(
    ...
    """Test documenting the flow."""
    pg_id = test_pg_with_processors["pg_details"]["id"]

    # Use the document_nifi_flow tool
    document_args = {
        "process_group_id": pg_id,
        "include_properties": True,
        "include_descriptions": True
    }
    doc_results_list = await call_tool(
        client=async_client,
        base_url=base_url,
    ...
    assert isinstance(doc_results_list, list) and len(doc_results_list) > 0, "Documentation should return results"
    documentation = doc_results_list[0]

    global_logger.info(f"Flow documentation for PG {pg_id}: {documentation}")

    assert documentation, "Documentation should not be empty"
    assert "flow_structure" in documentation, "Documentation should contain flow structure"
    
    # Check for processors in the flow structure
    found_log_attribute = False
    found_generate_flow_file = False
    expected_log_name = f"mcp-test-log-{mcp_headers['X-Nifi-Server-Id']}"
    expected_generate_name = f"mcp-test-generate-{mcp_headers['X-Nifi-Server-Id']}"

    # Look in both flow_structure and unconnected_components
    for component in documentation["flow_structure"]:
        if "name" in component:
            if expected_log_name == component["name"]:
                found_log_attribute = True
            elif expected_generate_name == component["name"]:
                found_generate_flow_file = True

    if "unconnected_components" in documentation:
        for processor in documentation["unconnected_components"].get("processors", []):
            if processor.get("name") == expected_log_name:
                found_log_attribute = True
            elif processor.get("name") == expected_generate_name:
                found_generate_flow_file = True

    assert found_log_attribute, f"Documentation should include LogAttribute processor with name {expected_log_name}"
    assert found_generate_flow_file, f"Documentation should include GenerateFlowFile processor with name {expected_generate_name}"
